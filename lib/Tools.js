...
const Tools = {
    ...
    DRAW_MAP_PNG: function (options, callback) {
        ...
        if(settings.crop_x1>=settings.crop_x2) settings.crop_x2 = Number.MAX_VALUE;
        if(settings.crop_y1>=settings.crop_y2) settings.crop_y2 = Number.MAX_VALUE;
                        ...
                        if (!err) {
                            Jimp.read(robotImagePath, function (err, robotImage) {
                                if (!err) {
                                    ...
                                    image.getBuffer("image/png", callback);
                                } else {
                                    callback(err);
                                }
                            });
