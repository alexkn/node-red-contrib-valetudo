...
RRMapParser.PARSE_BLOCK = function parseBlock(buf, offset, result) {
    ...
    let type = buf.readUInt16LE(0x00 + offset);
    let hlength = buf.readUInt16LE(0x02 + offset);
    let length = buf.readUInt32LE(0x04 + offset);
                    ...
                    if(val !== 0) {
                        ...
                        switch(val) {
                            case 1:
                                ...
                                break;
                            case 8:
                                // TODO: 8 is apparently never used in tested firmwares
                                parameters.pixels.obstacle_weak.push(coords);
                                break;
                            default: // firmware 2008 uses 7 older firmwares use 255
